...
int main(int argc, char* argv[]) {
  ...
  int pidArray[albumSize * 2];
  int statusArray[albumSize * 2];
  int count = 0;
  for (int i = 0; i < albumSize; i++) {
    pidArray[count++] = scaleImage(i, 10);
    pidArray[count++] = scaleImage(i, 25);
  }
  ...
  for (int i = 0; i < albumSize; i++) {
    ...
    waitpid(pidArray[2 * i], &statusArray[i * 2], WUNTRACED | WCONTINUED);

    if (!WIFEXITED(statusArray[i * 2])) {
    ...
    printf("How many degree do you want to rotate? degree or Y/N\n");
    ...
    if (strlen(degree) > 9) {
      ...
      strcpy(degree, "90");
    } else if (degree[0] == 'Y' || degree[0] == 'y') {
      strcpy(degree, "90");
    } else if (degree[0] == 'N' || degree[0] == 'n') {
      strcpy(degree, "0");
    }
    ...
    waitpid(pidArray[i * 2 + 1], &statusArray[i * 2 + 1], WUNTRACED | WCONTINUED);
    if (WIFEXITED(statusArray[i * 2 + 1])) {
